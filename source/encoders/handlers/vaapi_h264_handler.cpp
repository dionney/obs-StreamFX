...
#include "vaapi_h264_handler.hpp"
...
void vaapi_h264_handler::adjust_info(ffmpeg_factory* factory, const AVCodec* codec, std::string& id, std::string& name,
								   std::string& codec_id)
{
	name = "AMD VAAPI H.264/AVC (via FFmpeg)";
}

void vaapi_h264_handler::get_defaults(obs_data_t* settings, const AVCodec* codec, AVCodecContext* context, bool hw_encode)
{
	vaapi::get_defaults(settings, codec, context);
...
bool vaapi_h264_handler::has_keyframe_support(ffmpeg_factory* instance)
{
	return true;
}

bool vaapi_h264_handler::is_hardware_encoder(ffmpeg_factory* instance)
{
	return true;
}

bool vaapi_h264_handler::has_threading_support(ffmpeg_factory* instance)
{
	return false;
}

bool vaapi_h264_handler::has_pixel_format_support(ffmpeg_factory* instance)
{
	return false;
}

void vaapi_h264_handler::get_properties(obs_properties_t* props, const AVCodec* codec, AVCodecContext* context,
									  bool hw_encode)
{
	if (!context) {
...
void vaapi_h264_handler::update(obs_data_t* settings, const AVCodec* codec, AVCodecContext* context)
{
	vaapi::update(settings, codec, context);
...
void vaapi_h264_handler::override_update(ffmpeg_instance* instance, obs_data_t* settings)
{
	vaapi::override_update(instance, settings);
}

void vaapi_h264_handler::log_options(obs_data_t* settings, const AVCodec* codec, AVCodecContext* context)
{
	vaapi::log_options(settings, codec, context);
...
void vaapi_h264_handler::get_encoder_properties(obs_properties_t* props, const AVCodec* codec)
{
	vaapi::get_properties_pre(props, codec);
	...
	vaapi::get_properties_post(props, codec);
}

void vaapi_h264_handler::get_runtime_properties(obs_properties_t* props, const AVCodec* codec, AVCodecContext* context)
{
	vaapi::get_runtime_properties(props, codec, context);
}

void vaapi_h264_handler::migrate(obs_data_t* settings, std::uint64_t version, const AVCodec* codec,
							   AVCodecContext* context)
{
	vaapi::migrate(settings, version, codec, context);
}

void vaapi_h264_handler::override_colorformat(AVPixelFormat& target_format, obs_data_t* settings, const AVCodec* codec,
											AVCodecContext* context)
{
