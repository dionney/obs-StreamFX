...
#include "vaapi_hevc_handler.hpp"
#include "strings.hpp"
...
void vaapi_hevc_handler::adjust_info(ffmpeg_factory*, const AVCodec*, std::string&, std::string& name, std::string&)
{
	name = "AMD VAAPI H.265/HEVC (via FFmpeg)";
}

void vaapi_hevc_handler::get_defaults(obs_data_t* settings, const AVCodec* codec, AVCodecContext* context, bool)
{
...
bool vaapi_hevc_handler::has_keyframe_support(ffmpeg_factory*)
{
	return true;
}

bool vaapi_hevc_handler::is_hardware_encoder(ffmpeg_factory* instance)
{
	return true;
}

bool vaapi_hevc_handler::has_threading_support(ffmpeg_factory* instance)
{
	return false;
}

bool vaapi_hevc_handler::has_pixel_format_support(ffmpeg_factory* instance)
{
	return false;
}

void vaapi_hevc_handler::get_properties(obs_properties_t* props, const AVCodec* codec, AVCodecContext* context, bool)
{
	if (!context) {
...
void vaapi_hevc_handler::update(obs_data_t* settings, const AVCodec* codec, AVCodecContext* context)
{
...
void vaapi_hevc_handler::override_update(ffmpeg_instance* instance, obs_data_t* settings)
{
	vaapi::override_update(instance, settings);
}

void vaapi_hevc_handler::log_options(obs_data_t* settings, const AVCodec* codec, AVCodecContext* context)
{
...
void vaapi_hevc_handler::get_encoder_properties(obs_properties_t* props, const AVCodec* codec)
{
...
void vaapi_hevc_handler::get_runtime_properties(obs_properties_t* props, const AVCodec* codec, AVCodecContext* context)
{
	vaapi::get_runtime_properties(props, codec, context);
}

void streamfx::encoder::ffmpeg::handler::vaapi_hevc_handler::migrate(obs_data_t* settings, std::uint64_t version,
